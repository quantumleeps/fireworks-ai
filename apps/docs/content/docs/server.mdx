---
title: Server Guide
description: Endpoints, session context, permissions, thinking, persistence, and sandbox hooks for @neeter/server
---
...
<Img src="/images/thinking.png" alt="Extended thinking block with chain-of-thought reasoning" width={600} />
...
Each returned `{ name, value }` object is sent to the client as a `custom` SSE event. See [Custom events](/docs/client#custom-events) for the client-side handling.
...
<Img src="/images/permission-card.png" alt="Tool approval card with inline diff preview" width={600} />
...
<Img src="/images/session-resume.png" alt="Session history sidebar with resumable past conversations" width={600} />
...
> **Security note:** `createJsonSessionStore` writes conversation data — including tool inputs and outputs — to disk unencrypted. Use it for development and trusted environments. See the [code-workbench](https://github.com/quantumleeps/neeter/tree/main/examples/code-workbench) example for a complete persistence setup.
...
See the [code-workbench](https://github.com/quantumleeps/neeter/tree/main/examples/code-workbench) example for a complete sandboxing setup.
